...
/**
 * PATCH 8B — Implémentation service écarts caisse.
 *
 * Logique:
 * 1. Détection automatique variance (recettes, session)
 * 2. Investigation CONTROLEUR (notes, statut)
 * 3. Validation Chef de Bureau si seuil dépassé
 *
 * Seuils : gérés par EcartThresholdConfigService (paramétrable).
 * Aucun seuil n'est codé en dur dans cette classe ni dans l'entité.
 * La règle métier de seuil n'est pas définie dans les documents 3N fournis.
 *
 * Mapping rôles 3N → technique :
...
public class EcartCaisseServiceImpl implements EcartCaisseService {
    ...
    /** PATCH 8B — Seuils paramétrables, pas de valeur codée en dur. */
    private final EcartThresholdConfigService thresholdService;
    ...
    public EcartCaisseDTO detecterEcart(
        ...
        // PATCH 8B: seuilDepassé = flag explicite de l'appelant OU calcul via EcartThresholdConfigService
        // La décision finale repose toujours sur le service configurable, jamais sur une constante.
        boolean depasseConfig = thresholdService.necessiteValidationRCI(montantEcart);
        boolean finalSeuilDepassé = (seuilDepassé != null && seuilDepassé) || depasseConfig;
        ...
        EcartCaisse ecart = EcartCaisse.builder()
                ...
                .seuilDepassé(finalSeuilDepassé)
                .build();
    ...
    /**
     * PATCH 8 — Récupère tous les écarts, ordonnés par date décroissante
     */
    @Override
    public List<EcartCaisseDTO> getAll() {
        return ecartRepository.findAll().stream()
     ...
     */
    @Override
    public List<EcartCaisseDTO> getBySessionCaisseId(Long sessionId) {
        return ecartRepository.findBySessionCaisseIdOrderByDateCreationAsc(sessionId).stream()
                .map(ecartMapper::toDTO)
                .collect(Collectors.toList());
    }
    ...
    /**
     * PATCH 8 — Justification initiale d'un écart (CAISSIER ou CONTROLEUR).
     * Ajoute une note sans changer le statut.
     *
     * NOTE: La validation minimale 10 caractères est provisoire (technique).
     * Elle n'est pas définie dans les documents 3N fournis.
     */
    @Override
    public EcartCaisseDTO justifier(Long ecartId, String justification) {
        log.info("Justification écart: id={}", ecartId);
        ...
        if (justification == null || justification.isBlank() || justification.trim().length() < 10) {
            throw new BusinessException(
                ...
                "[validation technique provisoire — règle non définie dans les documents 3N]");
        }

        EcartCaisse ecart = ecartRepository.findById(ecartId)
                .orElseThrow(() -> new ResourceNotFoundException("Écart non trouvé: " + ecartId));

        if (ecart.getStatut() == StatutEcartCaisse.RESOLU ||
            ecart.getStatut() == StatutEcartCaisse.ACCEPTE ||
            ecart.getStatut() == StatutEcartCaisse.REJETE) {
            throw new BusinessException(
                "Impossible de justifier un écart avec statut: " + ecart.getStatut());
        }

        // Ajoute la justification aux notes d'investigation (append si existant)
        String existing = ecart.getNotesInvestigation();
        String updated = (existing != null && !existing.isBlank())
                ? existing + "\n[Justification]: " + justification.trim()
                : "[Justification]: " + justification.trim();
        ecart.setNotesInvestigation(updated);

        ecart = ecartRepository.save(ecart);
        log.info("Justification ajoutée pour écart: id={}, statut={}", ecartId, ecart.getStatut());
