...
import com.mini.credit.service.RecetteOperationGenerationService;
 ...
 * 4. PHASE 6B.2: Auto-genère les opérations (épargne, caisse)
 */
@Service
@Slf4j
@Transactional
...
public class RecetteJournaliereTerrainServiceImpl implements RecetteJournaliereTerrainService {
    ...
    private final RecetteOperationGenerationService operationGenerationService;
    ...
    public RecetteJournaliereTerrainDTO validerRecette(Long recetteId, BigDecimal cashRemis) {
        ...
        if (cashRemis == null) {
            throw new BusinessException("Le montant cash remis est obligatoire");
        }

        if (cashRemis.compareTo(BigDecimal.ZERO) < 0) {
            throw new BusinessException("Le montant cash remis doit être >= 0");
        }
        ...
        // PHASE 6B.2: Auto-genère les opérations (épargne, caisse)
        try {
            log.info("Triggering Phase 6B.2: Auto-generation of operations for recette {}", recetteId);
            RecetteOperationGenerationService.OperationGenerationSummaryDTO summary = 
                operationGenerationService.generateOperationsFromReceipt(recetteId);
            
            if (summary.success) {
                log.info("✓ Operations auto-generated successfully: {} epargne ops, {} caisse ops",
                        summary.operationEpargneCount, summary.operationCaisseCount);
            } else {
                log.warn("⚠ Operations auto-generation completed with warnings: {}", summary.message);
            }
        } catch (Exception e) {
            log.error("✗ Failed to auto-generate operations for recette {}: {}", 
                    recetteId, e.getMessage(), e);
            // Don't throw - operation generation is async-like and shouldn't block receipt validation
        }
     ...
     * (normalement automatique lors de la validation)
     */
    @Override
    public RecetteJournaliereTerrainDTO generateOperationsForReceipt(Long recetteId) {
        log.info("Triggering manual operation generation for recette: id={}", recetteId);

        RecetteJournaliereTerrain recette = recetteRepository.findById(recetteId)
                .orElseThrow(() -> new ResourceNotFoundException("Recette non trouvée: " + recetteId));

        // Vérification : la recette doit être validée
        if (recette.getStatut() != StatutRecetteJournaliere.VALIDEE) {
            throw new BusinessException("Seules les recettes validées peuvent générer des opérations. Statut actuel: " + recette.getStatut());
        }

        // Déclenche la génération d'opérations
        try {
            RecetteOperationGenerationService.OperationGenerationSummaryDTO summary =
                    operationGenerationService.generateOperationsFromReceipt(recetteId);

            log.info("Manual operation generation completed: success={}, epargne={}, caisse={}, message={}",
                    summary.success, summary.operationEpargneCount, summary.operationCaisseCount, summary.message);
        } catch (Exception e) {
            log.error("Error during manual operation generation for recette {}: {}", 
                    recetteId, e.getMessage(), e);
            throw new BusinessException("Erreur lors de la génération des opérations: " + e.getMessage());
        }

        // Retourne le détail actualisé de la recette
        return recetteMapper.toDTO(recette);
    }
