...
public class DemandeRetraitEpargne extends BaseEntity {
    ...
    @Column(name = "reference_retrait", unique = true, length = 20)
    private String referenceRetrait;

    @Column(name = "montant_demande", nullable = false, precision = 18, scale = 2)
    private BigDecimal montantDemande;

    @Column(name = "frais_retrait", nullable = false, precision = 18, scale = 2)
    @Builder.Default
    private BigDecimal fraisRetrait = BigDecimal.ZERO;

    @Column(name = "taux_commission_retrait", nullable = false, precision = 5, scale = 2)
    @Builder.Default
    private BigDecimal tauxCommissionRetrait = BigDecimal.ZERO;

    @Column(name = "montant_total_debite", nullable = false, precision = 18, scale = 2)
    @Builder.Default
    private BigDecimal montantTotalDebite = BigDecimal.ZERO;
    ...
    public boolean canBeValidated() {
        ...
        BigDecimal totalRequis = montantTotalDebite != null && montantTotalDebite.compareTo(BigDecimal.ZERO) > 0
            ? montantTotalDebite
            : montantDemande.add(fraisRetrait != null ? fraisRetrait : BigDecimal.ZERO);
        return compteEpargne.getSoldeDisponible() != null &&
            compteEpargne.getSoldeDisponible().compareTo(totalRequis) >= 0;
    }
