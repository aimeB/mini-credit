...
@Table(name = "recette_journaliere_terrain", indexes = {
        @Index(name = "idx_rjt_date_jour", columnList = "dateJour"),
        @Index(name = "idx_rjt_statut", columnList = "statut"),
        @Index(name = "idx_rjt_membre_id", columnList = "membre_id"),
        @Index(name = "idx_rjt_agent_id", columnList = "agent_id")
})
@Data
@NoArgsConstructor
@AllArgsConstructor
...
public class RecetteJournaliereTerrain extends BaseEntity {
    ...
    /**
     * PHASE 6B.1: Fiche journalière consolidée (optionnel, nullable pour backward compatibility)
     * Lien vers la fiche journalière synthèse
     * Phase 6B.2: phase de consolidation affectera les recettes à une fiche
     */
    @ManyToOne(fetch = FetchType.LAZY, optional = true)
    @JoinColumn(name = "fiche_journaliere_id", foreignKey = @ForeignKey(name = "fk_recette_fiche"))
    private FicheJournaliereAgentTerrain ficheJournaliere;
