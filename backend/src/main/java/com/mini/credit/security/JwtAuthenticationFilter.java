    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (jwt != null && jwtProvider.validateToken(jwt)) {
                ...
                Integer tokenCredentialsVersion = jwtProvider.getCredentialsVersionFromToken(jwt);
                JwtProvider.TokenCompatibilityResult tokenCompatibility = jwtProvider.inspectTokenCompatibility(jwt);
                ...
                if (userDetails instanceof Utilisateur utilisateur) {
                    Integer dbCredentialsVersion = utilisateur.getCredentialsVersion() == null ? 0 : utilisateur.getCredentialsVersion();
                    if (!dbCredentialsVersion.equals(tokenCredentialsVersion)) {
                        log.warn("❌ JWT REJETE credentialsVersion mismatch: username={} tokenVersion={} dbVersion={}", username, tokenCredentialsVersion, dbCredentialsVersion);
                        filterChain.doFilter(request, response);
                        return;
                    }
                }
