...
public interface MembreRepository extends JpaRepository<Membre, Long> {
    ...
    Optional<Membre> findByUtilisateur(Utilisateur utilisateur);
              ...
              @Query("""
                            SELECT COUNT(m) > 0
                            FROM Membre m
                            WHERE REPLACE(TRIM(m.telephonePrincipal), ' ', '') = :telephone
                                   AND (:excludedId IS NULL OR m.id <> :excludedId)
              """)
              boolean existsByNormalizedTelephonePrincipalExcludingId(@Param("telephone") String telephone, @Param("excludedId") Long excludedId);

              @Query("""
                            SELECT COUNT(m) > 0
                            FROM Membre m
                            WHERE UPPER(TRIM(m.prenom)) = :prenom
                                   AND UPPER(TRIM(m.nom)) = :nom
                                   AND (:excludedId IS NULL OR m.id <> :excludedId)
              """)
    ...
    List<Membre> findAllForAudit();
       ...
       @Query("SELECT m FROM Membre m " +
                 "WHERE m.statut != 'CLOTURE' " +
                 "AND (:siteId IS NULL OR m.site.id = :siteId) " +
                 "AND (:q IS NULL OR :q = '' OR " +
                 "LOWER(COALESCE(m.codeMembre, '')) LIKE LOWER(CONCAT('%', :q, '%')) OR " +
                 "LOWER(COALESCE(m.nomComplet, '')) LIKE LOWER(CONCAT('%', :q, '%')) OR " +
                 "LOWER(COALESCE(m.nom, '')) LIKE LOWER(CONCAT('%', :q, '%')) OR " +
                 "LOWER(COALESCE(m.telephonePrincipal, '')) LIKE LOWER(CONCAT('%', :q, '%')) OR " +
                 "LOWER(COALESCE(m.telephoneSecondaire, '')) LIKE LOWER(CONCAT('%', :q, '%')))")
       Page<Membre> searchActive(@Param("q") String q, @Param("siteId") Long siteId, Pageable pageable);
       ...
       @Query("""
              ...
              LEFT JOIN m.site s
              WHERE m.statut != 'CLOTURE'
                AND (:agenceId IS NULL OR s.agence.id = :agenceId)
              """)
       long countActiveByAgence(@Param("agenceId") Long agenceId);
}
