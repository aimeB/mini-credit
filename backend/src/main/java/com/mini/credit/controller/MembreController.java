...
public class MembreController {
    ...
    @GetMapping
    @Transactional(readOnly = true)
    @PreAuthorize("hasAuthority('MEMBRE_READ')")
    ...
    @Transactional(readOnly = true)
    @PreAuthorize("hasAuthority('MEMBRE_READ')")
    @Operation(summary = "Search members", description = "Recherche pagin̩e des membres actifs avec filtre texte et site")
    public Page<MembreResponse> search(
        ...
        if (size > 100) size = 100;
        Pageable pageable = PageRequest.of(page, size);
        return membreService.search(q, siteId, pageable);
    }
