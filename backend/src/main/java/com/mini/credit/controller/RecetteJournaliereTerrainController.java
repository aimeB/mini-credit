...
public class RecetteJournaliereTerrainController {
     ...
     */
    @PostMapping
    @PreAuthorize("hasAnyRole('ADMIN', 'CONTROLEUR', 'GESTIONNAIRE')")
     ...
     */
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'CONTROLEUR', 'AGENT_TERRAIN', 'GESTIONNAIRE')")
    ...
    /**
     * PHASE 6B.2: Endpoint pour déclencher manuellement la génération des opérations
     * Note: Normalement, la génération se fait automatiquement lors de la validation.
     * Cet endpoint est fourni comme fallback ou pour rafraîchir manuellement.
     */
    @PostMapping("/{id}/generate-operations")
    @PreAuthorize("hasAnyRole('ADMIN', 'CONTROLEUR')")
    @Operation(summary = "Manually trigger operation generation (PHASE 6B.2)", description = "Trigger auto-generation of operations for a validated receipt (normally automatic)")
    @Auditable(action = AuditAction.RECETTE_JOURNALIERE_VALIDATED, entityType = "RecetteJournaliereTerrain")
    public RecetteJournaliereTerrainDTO generateOperations(@PathVariable Long id) {
        return recetteService.generateOperationsForReceipt(id);
    }
}
