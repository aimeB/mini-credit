...
public class ReconciliationCaisseServiceImpl implements ReconciliationCaisseService {
     ...
     * Crée réconciliations pour sessions fermées avec écarts
     */
    ...
    public List<ReconciliationCaisseDTO> genererReconciliationsToutes() {
        ...
        // Cherche sessions fermées
        List<SessionCaisse> sessionsFermees = sessionCaisseRepository.findAll()
                .stream()
                .filter(s -> s.getStatut() == StatutSessionCaisse.FERMEE)
                .filter(s -> s.getSoldePhysique() != null)
                .filter(s -> s.getSoldePhysique().compareTo(s.getSoldeTheorique()) != 0)
                .collect(Collectors.toList());

        log.info("Nombre de sessions avec écarts détectés: {}", sessionsFermees.size());

        // Crée réconciliations pour chaque session
        List<ReconciliationCaisseDTO> reconciliations = sessionsFermees.stream()
                .map(session -> {
                    try {
    ...
    /**
     * Crée une réconciliation pour une session caisse fermée
     *
    ...
    public ReconciliationCaisseDTO creerReconciliation(Long sessionCaisseId) {
        ...
        // Vérifie que session est fermée
        if (session.getStatut() != StatutSessionCaisse.FERMEE) {
            throw new BusinessException("Session non fermée: " + sessionCaisseId);
        }
