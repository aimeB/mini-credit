...
public class CreditValidationServiceImpl implements CreditValidationService {
    ...
    /**
     * CRITÈRE 1: Frais demande payés > 0
     */
    @Override
    public boolean validerFreisDemandePayes(DemandeCredit demandeCredit) {
        if (demandeCredit == null) {
            return false;
        }
        BigDecimal fraisPayes = demandeCredit.getFraisDemandePayes();
        boolean isValid = fraisPayes != null && fraisPayes.compareTo(BigDecimal.ZERO) > 0;
        
        log.debug("Validation frais demande: demandeId={}, fraisPayes={}, isValid={}",
                demandeCredit.getId(), fraisPayes, isValid);
    ...
    public boolean validerGarantieDeposee(DemandeCredit demandeCredit) {
        ...
        // Cherche les garanties déposées pour cette demande
        List<Garantie> garanties = garantieRepository.findByDemandeCredit(demandeCredit);
        
        // Somme les montants bloqués
        BigDecimal totalGarantieBloquee = garanties.stream()
    ...
    public CreditValidationResult validerCreditComplet(DemandeCredit demandeCredit) {
        ...
        boolean fraisOk = validerFreisDemandePayes(demandeCredit);
        result.setFraisDemandePayesOk(fraisOk);
        result.setFraisDemandeMontant(demandeCredit.getFraisDemandePayes());
        if (!fraisOk) {
            result.setFraisDemandeMessage("Frais de demande non payés ou égaux à zéro");
        } else {
        ...
        if (result.isValid()) {
            ...
            if (!fraisOk) motif.append("Frais non payés | ");
    ...
    public BigDecimal calculerGarantieRequise(BigDecimal montantDemande) {
        ...
        // Récupère le paramètre POURCENTAGE_GARANTIE (20% par défaut)
        BigDecimal pourcentageGarantie = parametreMetierService.getDecimal("POURCENTAGE_GARANTIE");
        if (pourcentageGarantie == null || pourcentageGarantie.compareTo(BigDecimal.ZERO) <= 0) {
            // Fallback à 20% si paramètre non disponible
            pourcentageGarantie = BigDecimal.valueOf(20);
        }

        return montantDemande.multiply(pourcentageGarantie)
