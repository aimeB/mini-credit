...
public class DemandeRetraitEpargneController {
     ...
     */
    @PostMapping
    @PreAuthorize("hasAnyRole('MEMBER', 'AGENT_TERRAIN')")
    ...
    public DemandeRetraitEpargneDTO creerDemande(
        ...
        return demandeRetraitEpargneService.creerDemande(compteEpargneId, montant, observation);
    }
     ...
     */
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'CONTROLEUR', 'CAISSIER', 'MEMBER')")
    ...
    @Auditable(action = AuditAction.DEMANDE_RETRAIT_EPARGNE_DISBURSED, entityType = "DemandeRetraitEpargne")
