...
public class ContratCreditController {
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'RESPONSABLE', 'AGENT_BUREAU', 'AGENT_TERRAIN') or (hasRole('MEMBER') and @contratCreditService.isCurrentUserContract(#id))")
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'RESPONSABLE', 'AGENT_BUREAU', 'AGENT_TERRAIN') or (hasRole('MEMBER') and @contratCreditService.isCurrentUserCredit(#creditId))")
