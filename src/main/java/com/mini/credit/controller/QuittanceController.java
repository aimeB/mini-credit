...
public class QuittanceController {
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'RESPONSABLE', 'CAISSIER', 'AGENT_BUREAU', 'AGENT_TERRAIN') or (hasRole('MEMBER') and @quittanceService.isCurrentUserReceipt(#id))")
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'RESPONSABLE', 'AGENT_BUREAU', 'AGENT_TERRAIN') or (hasRole('MEMBER') and @quittanceService.isCurrentUserMembre(#membreId))")
